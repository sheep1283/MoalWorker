    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        dirFire.addValueEventListener(object: ValueEventListener {
            override fun onDataChange(p0: DataSnapshot) {
                if(want_to_join != null) {
                    want_to_join.setOnClickListener { //null exception 발생하여 null처리함
                        if (join_store.text.toString() == null) { //공백상태로 요청하기 누를 경우
                            Toast.makeText(activity, "검색할 가게 이름을 입력해주세요!", Toast.LENGTH_SHORT)
                                .show()
                        } else {
                        ...
                        for (snapShotStore: DataSnapshot in p0.child("stores").children) {
                            ...
                            if (snapShottedName == null) { //DB에 등록된 가게가 아예 없을 경우. 유저 사용시 이런 경우는 없을 것.
                                Toast.makeText(activity, "아직 가게 정보가 없어요!", Toast.LENGTH_SHORT)
                                    .show()
                            } else {
                                if (snapShottedName == registerStore) {
